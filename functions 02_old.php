...
function show_lesson_files_meta_box() {
    global $post;
    $attachmentMeta = unserialize(get_post_meta( $post->ID, 'lesson_attachments', true ));
    ...
    if($attachmentMeta) {
        ...
        $pptCustomTitle = isset($attachmentMeta['ppt']['title']) ? $attachmentMeta['ppt']['title'] : '';
        $pptURL = !empty($pptAttachmentId) ? wp_get_attachment_url( $pptAttachmentId ) : $attachmentMeta['ppt']['url'];

        $activityGuideTitle = !empty($activityGuideAttachmentId) ? basename ( get_attached_file( $activityGuideAttachmentId ) ) : '';
        $activityGuideCustomTitle = isset($attachmentMeta['activityGuide']['title']) ? $attachmentMeta['activityGuide']['title'] : '';
        $activityGuideURL = !empty($activityGuideAttachmentId) ? wp_get_attachment_url( $activityGuideAttachmentId ) : $attachmentMeta['activityGuide']['url'];

        $quizTitle = !empty($quizAttachmentId) ? basename ( get_attached_file( $quizAttachmentId ) ) : '';
        $quizCustomTitle = isset($attachmentMeta['quiz']['title']) ? $attachmentMeta['quiz']['title'] : '';
        $quizURL = !empty($quizAttachmentId) ? wp_get_attachment_url( $quizAttachmentId ) : $attachmentMeta['quiz']['url'];
    }
    ?>
...
function save_lesson_files_meta( $post_id ) {
    ...
    $lessonAttchments = serialize(
        array( 
            "ppt" => array(
                "pptID" => $pptAttachmentId,
                "title" => $pptCustomTitle,
                "url" => $pptAttachmentURL
            ),
            "activityGuide" => array(
                'activityGuideID' => $activityGuideAttachmentId,
                'title' => $activityGuideCustomTitle,
                "url" => $activityGuideAttachmentURL
            ),
            "quiz" => array(
                'quizID' => $quizAttachmentId,
                'title' => $quizCustomTitle,
                "url" => $quizAttachmentURL
            ) 
        )
    );
