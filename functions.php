...
function genesis_sample_google_fonts()
{
    ...
    wp_localize_script('script-js', 'wdm_script_obj', array('site_url_wdm' => site_url(), 'ajax_url' => admin_url('admin-ajax.php')));
    ...
    //csm certification page css
    wp_enqueue_style('csm-certification-css', get_stylesheet_directory_uri().'/css/csm_certification.custom.css', array());
    wp_enqueue_script('csm-certification-js', get_stylesheet_directory_uri().'/js/csm_certification_custom.js', array());
}
...
genesis_register_sidebar(array(
    'id'            => 'hp-6',
    'name'          => __('Homepage Section 6', 'ktwentyone'),
    'description'   => __('This is a widget area for Section 6 of Homepage', 'ktwentyone'),
    'before_title'  => '<div class="wdm-widget-title-wrap"><h4 class="widget-title widgettitle">',
    'after_title'   => "</h4></div>"
));
...
genesis_register_sidebar(array(
    'id'            => 'hp-7',
    'name'          => __('Homepage Section 7', 'ktwentyone'),
    'description'   => __('This is a widget area for Section 7 of Homepage', 'ktwentyone'),
    'before_title'  => '<div class="wdm-widget-title-wrap"><h4 class="widget-title widgettitle">',
    'after_title'   => "</h4></div>"
));
    ...
    <div class="modal-dialog">
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
            <p class="ajax-response hidden"></p>
...
add_action( 'wp_ajax_k21_login_handler', 'k21_login_handler' );
add_action( 'wp_ajax_nopriv_k21_login_handler', 'k21_login_handler' );
function k21_login_handler() {
    $start = microtime(true);
    $info = array();
    $info['user_login'] = sanitize_text_field(trim($_POST['log']));
    $info['user_password'] = trim($_POST['pwd']);
    $info['remember'] = isset($_POST['remember-me']) ? true : false;
    $redirect_url = isset($_POST['redirect_to']) ? home_url($_POST['redirect_to']) : '';

    if ( !$info['user_login'] ) {
        wp_send_json_error(array('message' => '<strong>Error</strong>: Username or E-mail is required.', 'for'=>'username'));
    }

    if ( !$info['user_password'] ) {
        wp_send_json_error(array('message' => '<strong>Error</strong>: Password is required.', 'for'=>'password'));
    }
    ...
    if ( is_wp_error($user_signon) ){
        ...
        $message = "<strong>Success</strong>: Logged In successfully. Redirecting...";
